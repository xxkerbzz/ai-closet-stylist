...
// In development: process.cwd() is 'web', so we go up one level
// In production build: process.cwd() is still 'web', same logic applies
const CONTENT_DIR = path.isAbsolute(config.contentDirectory)
  ? config.contentDirectory
  : path.resolve(process.cwd(), '..', config.contentDirectory.replace('../', ''));
