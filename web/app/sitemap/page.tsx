...
function parseSitemapXml(xmlText: string): SitemapUrl[] {
  ...
  for (let i = 0; i < urlElements.length; i++) {
    ...
    try {
      ...
      urls.push({ loc: loc.replace(/^https?:\/\/[^/]+/, '') || loc, lastmod, changefreq, priority });
    }
  }
...
function organizeUrls(urls: SitemapUrl[]): SitemapSection[] {
  ...
  const corePages = urls.filter(url => {
    const path = url.loc;
    return path === '/' || path.startsWith('/about') || path.startsWith('/contact') || 
           path.startsWith('/faq') || path.startsWith('/product') || path.startsWith('/features') ||
           (path.startsWith('/resources') && path === '/resources');
  });
  
  if (corePages.length > 0) {
    sections.push({ title: 'Main Pages', urls: corePages });
  }
  ...
  if (legalPages.length > 0) {
    sections.push({ title: 'Legal Pages', urls: legalPages });
  }
  ...
  Object.entries(pillarGroups).forEach(([pillar, pillarUrls]) => {
    ...
    sections.push({ title: formattedTitle, urls: pillarUrls });
  });
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mt-4">
              ...
              <p className="text-lg text-gray-600 mb-4">
                Browse all pages on AI Closet Stylist. This sitemap is generated from our XML sitemap used by search engines.
              </p>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Last Modified</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Change Frequency</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {section.urls.map((url, urlIndex) => (
                            <tr key={urlIndex} className="hover:bg-gray-50 transition-colors">
                              <td className="px-4 py-3 whitespace-nowrap">
                                <Link href={url.loc} className="text-blue-600 hover:text-blue-700 hover:underline font-medium flex items-center gap-2 group">
                                  <svg className="w-4 h-4 text-gray-400 group-hover:text-blue-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                  </svg>
                                  <span>{getPageTitle(url.loc)}</span>
                                </Link>
                                <div className="text-xs text-gray-500 mt-1 md:hidden">{url.loc}</div>
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 hidden md:table-cell">{formatDate(url.lastmod)}</td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 hidden lg:table-cell">
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                  {url.changefreq || 'N/A'}
                                </span>
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 hidden lg:table-cell">{url.priority || 'N/A'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
