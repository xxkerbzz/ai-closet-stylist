...
${urls.map((url) => {
  ...
  return `  <url>
    ...
    const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || 
      (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://aiclosetstylist.com');
    const corePages = [
      ...
      { loc: `${BASE_URL}/product`, lastmod: new Date(), changefreq: 'weekly' as const, priority: 0.9 },
